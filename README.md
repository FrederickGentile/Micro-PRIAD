...
#### ARGS.txt
The `ARGS.txt` file contains the same informations that you would need to define in Julia if you chose the execution option 2, 3 or 4. It contains arguments described in [ARGS_README](/Micro-PRIAD/Documentation/BB_Parameter/ARGS_README.md) and formated like in the `ex_ARGS.txt` files located in each folder in `$MICRO_PRIAD_HOME/Tests`.
...
```
The `x.txt` file must contain only the numerical value of each variable seperated with spaces without "[", "," or "]", the `x0_feasible` and `x0_infeasible` folder contains good example of how to formate your `x.txt` file, you will find those files if you go as deep as you can in the `$MICRO_PRIAD_HOME/Tests` directory ([input length of 28 exemple](/Micro-PRIAD/Tests/instance=1/length_input=28/x0_feasible/1.txt)).

For all integer input the bounds are 1 and 9 and for all reel input the bounds are 0.1 and 10.0.

## Micro-PRIAD's Behavior
### Running time 
The Running time for each input length of each instance at different fidelity is logged in the [BEHAV_README](/Micro-PRIAD/Documentation/Behaviour/BEHAV_README.md). 
### Best objective function value found
A list of the best known objectiv function value is also logged in the [BEHAV_README](/Micro-PRIAD/Documentation/Behaviour/BEHAV_README.md). 
### General Behavior
A general behavior description is done in the [BEHAV_README](/Micro-PRIAD/Documentation/Behaviour/BEHAV_README.md). 
## Other tools

In the `$MICRO_PRIAD_HOME/Benchmarking_Tools` directory you will find different julia file that you can execute to run on each instance for each input length  `NOMAD`, an `LHS` (Latin Hypercube Sampling) or a `RS` (Random Search). the command necessary to run those methods are described in the [TOOLS_README](/Micro-PRIAD/Documentation/Other_Tools/TOOLS_README.md).

----------------------------

The tree structure of the repo is shown in the [TREE_STRUCT_README](/Micro-PRIAD/Documentation/TREE_STRUCT_README.md)
